...
/* ==========================================================================
   8. GENERIC UPDATE EMPLOYEE (PATCH /:employeeId)
   Handles Name, Email, Dept, Designation, Manager, etc.
   ========================================================================== */
router.patch("/:employeeId", verifyToken, async (req, res) => {
    ...
    try {
        ...
        const { employeeId } = req.params;
        const { name, email, department, designation, manager_id, client_name, work_location } = req.body;

        // Find linked user ID
        const [emp] = await connection.query("SELECT user_id FROM employees WHERE id = ?", [employeeId]);
        if (!emp.length) {
            connection.release();
        ...
        // 1. Update USERS Table (Name, Email) - Critical for Login/Display
        // FIX: Added this block to ensure editing name/email in modal works
        if (name || email) {
            const userUpdates = [];
            const userValues = [];
        ...
        // 2. Update EMPLOYEES Table (Profile Data)
        const empUpdates = [];
        const empValues = [];

        if (name) { empUpdates.push("name = ?"); empValues.push(name); }
        if (email) { empUpdates.push("email = ?"); empValues.push(email); }
        if (department) { empUpdates.push("department = ?"); empValues.push(department); }
        if (designation !== undefined) { empUpdates.push("designation = ?"); empValues.push(designation); }
        if (client_name) { empUpdates.push("client_name = ?"); empValues.push(client_name); }
        if (work_location) { empUpdates.push("work_location = ?"); empValues.push(work_location); }
        
        if (manager_id !== undefined) {
             if (manager_id && String(manager_id) === String(employeeId)) {
                 await connection.rollback();
                 connection.release();
        ...
        if (empUpdates.length > 0) {
            ...
            await connection.query(`UPDATE employees SET ${empUpdates.join(", ")} WHERE id = ?`, empValues);
        }
