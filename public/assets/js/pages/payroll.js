...
function initPayroll() {
  ...
  // Replaced with async wrapper to handle the loading state
  downloadBtn.addEventListener("click", async (e) => {
    e.preventDefault();
    await downloadPDF(downloadBtn);
  });
...
async function loadPayslip(month) {
  try {
    const data = await apiGet(`/payslips/my/${encodeURIComponent(month)}`);
...
/* =========================
   DOWNLOAD PDF
========================= */
async function downloadPDF(btnElement) {
  const month = document.getElementById("month").value;
  if (!month) return alert("Select a month");

  // CRITICAL FIX: Prevent spam clicking while Puppeteer generates the PDF
  const originalText = btnElement.innerText;
  btnElement.innerText = "Generating PDF...";
  btnElement.disabled = true;

  try {
    const res = await fetch(`/api/payslips/my/${encodeURIComponent(month)}/pdf`, {
      method: 'GET',
      headers: {
        "Authorization": "Bearer " + localStorage.getItem("token"),
        "Accept": "application/pdf" // Tell the backend we expect a PDF
      }
    });
    ...
    if (blob.size < 1000) {
      throw new Error("Invalid or empty PDF received from server");
    }
    ...
    console.error(err);
    alert("Unable to download payslip PDF. Please try again.");
  } finally {
    // Restore button state no matter what happens
...
function enablePayrollUploadForHR() {
  try {
    ...
    if (["admin", "hr"].includes(user.role)) {
      document.getElementById("payrollUploadWrapper")?.classList.remove("d-none");
    }
  } catch {}
}
...
function bindPayrollUpload() {
  ...
  const msg = document.getElementById("uploadMsg") || document.getElementById("msg");
  ...
  form.addEventListener("submit", async e => {
    ...
    try {
      ...
      msg.innerHTML = `<div class="alert alert-danger">Payroll upload failed</div>`;
    }
  });
}
